...
from transformers import FlaxAutoModelForSequenceClassification, AutoModelForSequenceClassification
...
def match_params(model1, model2, return_models="pt", head_perm=True):
    ...
    params1 = pd.json_normalize(jtu.tree_map(lambda x: x, model1.params), sep="/").to_dict(orient="records")[0]
    params2 = pd.json_normalize(jtu.tree_map(lambda x: x, model2.params), sep="/").to_dict(orient="records")[0]
    ...
    if head_perm:
        perm = head_perm_weight_matching(rng, ps, params1, params2,)
    else:
        perm = weight_matching(rng, ps, params1, params2, masks=masks)
    
    permuted_params2 = apply_permutation(ps, perm, params2,)
    nested_permuted_params2 = flat_dict_to_nested(permuted_params2)
    model2.params = nested_permuted_params2
    
    if return_models=="pt":
        model1_dir_name, model2_dir_name = ''.join(random.choices(string.ascii_letters, k=20)) 
        model1.save_pretrained(model1_dir_name)
        model1 = AutoModelForSequenceClassification.from_pretrained(model1_dir_name, from_flax=True)
        model2.save_pretrained(model2_dir_name)
        model2 = AutoModelForSequenceClassification.from_pretrained(model2_dir_name, from_flax=True)
